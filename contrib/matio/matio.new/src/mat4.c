...
{
    unsigned int N;
    if ( matio_seek(mat->fp,matvar->internal->datapos,SEEK_SET) )
        return;
        ...
        case MAT_T_DOUBLE:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                    ...
                    matio_read(complex_data->Re,N,sizeof(double),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(double),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(double);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(double),mat->fp);
            }
            break;
        case MAT_T_SINGLE:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                     complex_data->Re != NULL && complex_data->Im != NULL ) {
                    matio_read(complex_data->Re,N,sizeof(float),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(float),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(float);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(float),mat->fp);
            }
            break;
        case MAT_T_INT32:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                     complex_data->Re != NULL && complex_data->Im != NULL ) {
                    matio_read(complex_data->Re,N,sizeof(mat_int32_t),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(mat_int32_t),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(mat_int32_t);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(mat_int32_t),mat->fp);
            }
            break;
        case MAT_T_INT16:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                     complex_data->Re != NULL && complex_data->Im != NULL ) {
                    matio_read(complex_data->Re,N,sizeof(mat_int16_t),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(mat_int16_t),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(mat_int16_t);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(mat_int16_t),mat->fp);
            }
            break;
        case MAT_T_UINT16:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                     complex_data->Re != NULL && complex_data->Im != NULL ) {
                    matio_read(complex_data->Re,N,sizeof(mat_uint16_t),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(mat_uint16_t),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(mat_uint16_t);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(mat_uint16_t),mat->fp);
            }
            break;
        case MAT_T_UINT8:
            ...
            if ( matvar->isComplex ) {
                ...
                if ( complex_data != NULL &&
                     complex_data->Re != NULL && complex_data->Im != NULL ) {
                    matio_read(complex_data->Re,N,sizeof(mat_uint8_t),mat->fp);
                    matio_read(complex_data->Im,N,sizeof(mat_uint8_t),mat->fp);
                }
            } else {
                matvar->nbytes = N*sizeof(mat_uint8_t);
                matvar->data   = malloc(matvar->nbytes);
                if ( matvar->data != NULL )
                    matio_read(matvar->data,N,sizeof(mat_uint8_t),mat->fp);
            }
            break;
        default:
      ...
      int *start,int *stride,int *edge)
{
    ...
    if ( matvar->rank == 2 ) {
        ...
        if ( matvar->isComplex ) {
            ...
            matio_seek(mat->fp,matvar->internal->datapos+nbytes,SEEK_SET);
...
{       
    ...
    err = matio_read(&tmp,sizeof(int),1,mat->fp);
    if ( !err ) {
        free(matvar);
        return NULL;
    }
    ...
    err = matio_read(&(matvar->isComplex),sizeof(int),1,mat->fp);
    if ( !err ) {
        Mat_VarFree(matvar);
        return NULL;
    }
    ...
    err = matio_read(matvar->name,1,tmp,mat->fp);
    if ( !err ) {
        Mat_VarFree(matvar);
        return NULL;
    }
    ...
    if ( matvar->isComplex )
        nBytes *= 2;
    matio_seek(mat->fp,nBytes,SEEK_CUR);
