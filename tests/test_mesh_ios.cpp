...
int are_equal(const Mesh &m1, const Mesh &m2, double tol = 1e-12) {
  if ((m1.vertices().size() != m2.vertices().size()) ||
      (m1.triangles().size() != m2.triangles().size()))
    return 0;

  for (auto vit1 = m1.vertices().begin(), vit2 = m2.vertices().begin();
       vit1 != m1.vertices().end(); vit1++, vit2++)
    if (!are_equal(**vit1, **vit2, tol))
      return 0;

  for (Triangles::const_iterator tit1 = m1.triangles().begin(),
                                 tit2 = m2.triangles().begin();
       tit1 != m1.triangles().end(); ++tit1, ++tit2)
    for (Triangle::const_iterator sit1 = tit1->begin(), sit2 = tit2->begin();
         sit1 != tit1->end(); ++sit1, ++sit2)
