...
- HeadNNcX are versions of HeadNNaX with a different (finer) discretization close to the two hemispheres borders: north & south. ( with X={1,2,3})
         HeadNNc1 and HeadNNc3 were generated using CGAL (via meshing tools that OpenMEEG used to ship), while model HeadNNc2 is one of the head models used in the article of Kybic et al [1]_ ( with X={1,2,3})
...
Using the tools:
^^^^^^^^^^^^^^^^
Models such as *HeadNNc1* and *HeadNNc3* were generated by meshing an implicit hemisphere
function with CGAL_, then mirroring the northern hemisphere to obtain the southern one with
matching vertices at their interface. The CGAL-based meshing tools have since been removed
from OpenMEEG; equivalent meshes can be produced with CGAL directly, or from the Python
stack (e.g. ``skimage.measure.marching_cubes`` plus PyVista_ decimation).
