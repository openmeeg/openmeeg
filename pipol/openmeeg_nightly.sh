...
#PIPOL esn amd64_kvm-linux-debian-lenny  none 2:40 --silent --root
#PIPOL esn amd64-linux-fedora-core11.dd.gz none 2:40 --silent --root
...
rm -f CMakeCache.txt
cmake -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON -DENABLE_PACKAGING=ON -DENABLE_PYTHON=ON -DUSE_ATLAS=OFF -DUSE_MKL=OFF
build

## MKL (static mode)
#rm -f CMakeCache.txt
#cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DENABLE_PACKAGING=ON -DUSE_ATLAS=OFF -DUSE_MKL=ON -DBUILD_SHARED_LIBS=OFF -DUSE_PROGRESSBAR=ON .
#build
