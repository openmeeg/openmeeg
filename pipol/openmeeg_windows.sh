...
function build_and_test() {
    ...
    if [ $SYSTEMCOMPILE -eq "win32" ]; then  # don't use OpenMP on windows 32bits
        ${CMAKE} -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DENABLE_PACKAGING=ON -DUSE_OMP=OFF -DMATLAB_TESTING=OFF -DMATIO_BUILD_TESTING=OFF -G "$PIPOL_WIN_COMPILER$SYSTEMOS" ..
    else
        ${CMAKE} -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=ON -DENABLE_PACKAGING=ON -DUSE_OMP=ON -DMATLAB_TESTING=OFF -G "$PIPOL_WIN_COMPILER$SYSTEMOS" ..
    fi
