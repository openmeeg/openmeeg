...
OpenMEEG handles several file formats corresponding to several type of objects: vectors, matrices, head geometries, meshes, dipoles, conductivities.
    ...
    If {\tt MeshFile} is found, it specifies the path to the VTK/vtp file containing the vertices, and annoted triangles of your geometry. (Annotaion of the triangles are labels which will define the mesh names).\\
    If {\tt Meshes} is found, it specifies the path to the meshes that can be named or not. Mesh file formats supported are tri, bnd, mesh, off, and vtk (in case you use VTK).
    A Mesh is defined with the keyword {\tt Mesh} followed by a name and "{\tt :}".
    In case no name is provided, the Mesh will get an index as name (starting from 1).
    
    If both are not found, the next section called {\tt Intefaces} will expect the filenames of the meshes.
    ...
    If section {\tt MeshFile} or {\tt Meshes} were NOT specified before, a path to a mesh file is expected.
    Else a sequence of mesh names are expected.
    ...
    Each domain is then depicted, one domain per line, by the keyword {\tt Domain} followed by the domain name (which will serve for identification and will also appear in the conductivity description) followed by a list of IDs (names or integers). 
    These IDs are the interface names (as depicted in previous paragraph). 
    They are affected by a + or - sign depending on whether the domain is outside or inside the corresponding interface (as defined by the outward normal of the interface).
