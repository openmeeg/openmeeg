#ifndef MYSPARSELIB_H
#define MYSPARSELIB_H
...
template<class T,class I> class MySparseLib: public TgenericMatrix<T>
{
public:
    typedef T valType;
    typedef I idxType;
...
public:
    ...
    inline cellType ** const getColEntry() const {return _ColEntry;}
    inline cellType ** const getRowEntry() const {return _RowEntry;}
    ...
    inline MySparseLib(idxType n,idxType p, sparsity sp=column)
    {
        init(n,p,sp);
    }
    ...
    inline MySparseLib ( const MySparseLib<T,I>& modele )
    {
    ...
    inline MySparseLib<T,I> operator * (  MySparseLib<T,I>& opd)
    {
    ...
    inline void saveBin ( const char * filename) const // Les colonnes sont ecrites les unes ‡ la suite des autres
    {
        ...
        idxType nn=this->_n;
        idxType pp=this->_p;
        fwrite(&nn,sizeof(idxType),1,fichier);
        fwrite(&pp,sizeof(idxType),1,fichier);
        ...
        for(idxType j=0;j<_p;j++)
        {
            if(_ColEntry[j]!=0) // Si la colonne n'est pas vide, il y a quelque chos ‡ ecrire
            {
                ...
                while(ce!=0)
                {
                    fwrite(&(ce->i),sizeof(idxType),1,fichier);
                    fwrite(&(ce->j),sizeof(idxType),1,fichier);
                    fwrite(&(ce->val),sizeof(T),1,fichier);
                    ce=ce->down;
                }
            }
    ...
    inline void loadTxt( const char * fileName) // Les colonnes sont ecrites les unes ‡ la suite des autres
    {
        ...
        // On ecrit avant toute chose le nombre de lignes et de colonnes de la matrice
        int nnn,ppp,iii,jjj;
        fscanf(fichier,"%d",&nnn);
        fscanf(fichier,"%d",&ppp);
        ...
        _nz=0;
        int nread=1;
        fscanf(fichier,"%d",&iii);
        fscanf(fichier,"%d",&jjj);
        fscanf(fichier,"%lf",&vval);
        ii=iii;jj=jjj;
        while(nread>0)
        {
            _nz++;
            ...
            nread=0;
            idxType oldjj=jj;
            nread+=fscanf(fichier,"%d",&iii);
            nread+=fscanf(fichier,"%d",&jjj);
            ii=iii; jj=jjj;
            nread+=fscanf(fichier,"%lf",&vval);
    ...
    inline void loadBin( const char * fileName) // Les colonnes sont ecrites les unes ‡ la suite des autres
    {
        ...
        // On ecrit avant toute chose le nombre de lignes et de colonnes de la matrice
        fread(&nn,sizeof(idxType),1,fichier);
        fread(&pp,sizeof(idxType),1,fichier);
        ...
        _nz=0;
        size_t nread=1;
        fread(&ii,sizeof(idxType),1,fichier);
        fread(&jj,sizeof(idxType),1,fichier);
        fread(&vval,sizeof(T),1,fichier);
        while(nread>0)
        {
            _nz++;
            ...
            nread=0;
            idxType oldjj=jj;
            nread+=fread(&ii,sizeof(idxType),1,fichier);
            nread+=fread(&jj,sizeof(idxType),1,fichier);
            nread+=fread(&vval,sizeof(T),1,fichier);
    ...
    {
        ((MySparseLib<T,I>*)this)->refreshNZ();
    ...
    inline MySparseLib ( const char * filename, bool ascii=false) // Les colonnes sont ecrites les unes ‡ la suite des autres
    {
        init();
        if(ascii) loadTxt(filename);
        else loadBin(filename);
    }
    ...
    inline void display() const //Procedure lente car pas optimisee!!
    {
        ...
        {
            double val=((const MySparseLib<T,I> &)(*this))(i,j);
            if(val!=0)
            {
                flag=false;
    ...
    MySparseLib<T,I> operator * (  MySparseLib<T,I>& opd)
    {
        ...
        {
            cellType* startCol=(opd.ColEntry)[j];
            cellType* startRow=RowEntry[i];
            if(startCol!=0 && startRow!=0)
            {
                T total=0;
                while(startRow!=0)
    ...
    */
    ...
    /*
    //  Version avec Construction en ligne
    ...
    //On insere en debut de ligne
                        ...
                        {
    //On insere ‡ la fin d'une ligne
                            newCell->left=voisinGauche;
                            voisinGauche=newCell;
                        }

    //On insere la nouvelle cellule dans la colonne
                        if(voisinsHaut[j]==0)
                        {
    //On insere en debut de ligne
                            newCell->left=0;
                        ...
                        {
    //On insere ‡ la fin d'une ligne
                            newCell->left=voisinsHaut[j];
                            voisinsHaut[j]=newCell;
                        }
                    } // Fin de l'insertion d'une nouvelle cellule
    ...
    */
    ...
    inline void operator += ( MySparseLib<T,I>& opd)
    {
        ...
        //MySparseLib<T,I> *this(this->n,this->p);
    ...
    inline void operator = (MySparseLib<T,I>& modele)
    {
...
template<class T,class I> inline std::ostream& operator<<(std::ostream& f,const MySparseLib<T,I> &M)
{
    // On parcourt les colonnes
    for(size_t j=0;j<M.ncol();j++)
    {
        if(((MySparseLib<T,I> &)M).getColEntry()[j]!=0) // Si la colonne n'est pas vide, il y a quelque chos ‡ ecrire
        {
            typename MySparseLib<T,I>::cellType *ce=((MySparseLib<T,I> &)M).getColEntry()[j];
            while(ce!=0)
            {
