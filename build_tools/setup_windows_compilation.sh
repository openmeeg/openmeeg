...
if [[ "$VCPKG_DEFAULT_TRIPLET" == "x64-mingw-dynamic" ]]; then
    ...
    if [[ "$CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM" == "" ]]; then
        CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM="10.0.14393.0"
    fi
    export CMAKE_GENERATOR="$CMAKE_GENERATOR"
    export CMAKE_GENERATOR_PLATFORM="x64"
    export CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM="$CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM"
    export SDK_OPT="-DCMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM=$CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION_MAXIMUM"
