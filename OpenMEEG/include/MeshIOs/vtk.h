    ...
    public:
        ...
        void load_triangles(OpenMEEG::Mesh& this_mesh) override {
            ...
            for (unsigned i = 0; i < ntrgs; ++i) {
                ...
                this_mesh.add_triangle(TriangleIndices(l->GetId(0),l->GetId(1),l->GetId(2)),indmap);
            }
        }
    #else
        ...
        void save(const OpenMEEG::Mesh& this_mesh,std::ostream& os) const override {
            ...
            os << "POINTS " << this_mesh.vertices().size() << " float" << std::endl;

            const VertexIndices& vertex_index(this_mesh);
            for (const auto& vertex : this_mesh.vertices())
                os << *vertex << std::endl;

            os << "POLYGONS " << this_mesh.triangles().size() << ' ' << this_mesh.triangles().size()*4 << std::endl;
            ...
            os << "CELL_DATA " << this_mesh.triangles().size() << std::endl;
            os << "POINT_DATA " << this_mesh.vertices().size() << std::endl;
            os << "NORMALS normals float" << std::endl;
            for (const auto& vertex : this_mesh.vertices())
                os << this_mesh.normal(*vertex) << std::endl;
        }
