...
if (MKL_ROOT_DIR)
    ...
    # first the libs
    set(MKL_LINK_TOOL "${MKL_LINK_TOOL} -libs")
    ...
    if (MKL_INCLUDE_DIR MATCHES "Composer.*2013")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=ComposerXE2013")
    elseif (MKL_INCLUDE_DIR MATCHES "Composer.*2011")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=ComposerXE2011")
    elseif (MKL_INCLUDE_DIR MATCHES "10.3")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=10.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2013") # version 11 ...
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.1")
    elseif(MKL_INCLUDE_DIR MATCHES "2015")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.2")
    elseif(MKL_INCLUDE_DIR MATCHES "2016")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2017")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2018")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.3")
    elseif (MKL_INCLUDE_DIR MATCHES "10")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=10.2")
    else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --mkl=11.3")
    endif()

    if (CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --compiler=clang")
	elseif(CMAKE_CXX_COMPILER_ID STREQUAL "Intel")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --compiler=intel_c")
	elseif(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --compiler=ms_c")
	else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --compiler=gnu_c")
    endif()

    if (APPLE)
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --os=mac")
    elseif(WIN32)
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --os=win")
    else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --os=lnx")
    endif()

	set(MKL_LIB_DIR)
    if (${CMAKE_SIZEOF_VOID_P} EQUAL 8 AND NOT FORCE_BUILD_32BITS)
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --arch=intel64")
		set(MKL_LIB_DIR "intel64")
    else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --arch=ia-32")
    ...
    if (MKL_USE_sdl)
        ...
        if (BUILD_SHARED_LIBS AND NOT FORCE_STATIC_MKL)
            ...
            set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --linking=static")
        endif()
    endif()

    if (MKL_USE_parallel)
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --parallel=yes")
    else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --parallel=no")
    endif()

    if (FORCE_BUILD_32BITS)
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --interface=cdecl")
        set(MKL_USE_interface "cdecl" CACHE STRING "disabled by FORCE_BUILD_32BITS" FORCE)
    else()
        set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --interface=${MKL_USE_interface}")
    endif()

    if (MKL_USE_parallel)
        if (USE_OMP)
            set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --openmp=gomp")
        else()
            set(MKL_LINK_TOOL "${MKL_LINK_TOOL} --threading-library=iomp5 --openmp=iomp5")
        endif()
    endif()
    ...
    if (WIN32)
        ...
        # remove unwanted break
		string(REGEX REPLACE "\n" "" RESULT_LIBS ${RESULT_LIBS}) 
        if (MKL_LINK_TOOL MATCHES "static")
    ...
    # now definitions
    string(REPLACE "-libs" "-opts" MKL_LINK_TOOL "${MKL_LINK_TOOL}")
    execute_process(COMMAND ${MKL_LINK_TOOL} OUTPUT_VARIABLE RESULT_OPTS TIMEOUT 2 ERROR_QUIET)
