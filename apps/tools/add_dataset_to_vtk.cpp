...
int usage(const std::string progname) {
  std::cerr << "Usage:" << std::endl
            << "\t" << progname
            << " -m <vtk polydata file> -n <data field name> [data file] (-o "
...
int main(int argc, char *argv[]) {
  ...
  for (unsigned j = 0; j < data.ncol(); ++j) {
    ...
    if (data.ncol() > 1) {
      // dataname << "-" << std::setw(unsigned(log10(data.ncol())+1)) <<
      // std::setfill('0') << j; with 001 instead of 1
      dataname << "-" << j;
    }
    array->SetName(dataname.str().c_str());

    if (data.nlin() == (size_t)nbPoints)
      mesh->GetPointData()->AddArray(array);
    else if (data.nlin() == (size_t)nbCells)
      mesh->GetCellData()->AddArray(array);
    else {
      std::cerr << "Something's wrong ! got a mesh with " << nbPoints
                << " points and " << nbCells << " cells, and (" << data.nlin()
                << "," << data.ncol()
                << ") data. Don't know what to do with them !" << std::endl;
      return usage(argv[0]);
    }
  }
