...
struct Planes
{
    ...
    Planes(const OpenMEEG::Matrix &_mat,const double _fs): mat(_mat),fs(_fs) {}
...
struct Spheres {
    ...
    Spheres(const OpenMEEG::Matrix &_mat,const double _fs): mat(_mat),fs(_fs) {}
...
namespace OpenMEEG {
    ...
    Mesh cgal_refine(const Mesh& m_in,const double radius_bound,const double distance_bound,const char* sizing_field) {
        // Mesh criteria
