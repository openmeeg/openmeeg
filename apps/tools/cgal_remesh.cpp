...
int main(int argc,char **argv) {
    ...
    const CommandLine cmd(argc,argv,"Re-mesh a mesh:");
    ...
    Mesh mesh(input_filename,false);
    std::cout << "Input surface:\n nb of points: " << mesh.vertices().size() << "\t nb of triangles:\t" << mesh.triangles().size() << std::endl;

    const Mesh& refined_mesh = cgal_refine(mesh,radius_bound,distance_bound,sizing_field);
